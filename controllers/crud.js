...
// 查询数量
const getCount = (queryConditions = {}) => async (req, res, next) => {
  try {
    const doc = await req.Model.countDocuments().where(queryConditions)
...
// 查询所有符合条件的数据
const getAll = (options = {}) => async (req, res, next) => {
  try {
    const { queryConditions = {}, queryOptions = {} } = options
    const doc = await req.Model.find().where(queryConditions).setOptions(queryOptions)
...
// 详情查询
const getOnelById = (queryOptions = {}) => async (req, res, next) => {
  try {
    const doc = await req.Model.findById(req.params.id).setOptions(queryOptions)
    assert(doc, 400, { message: '获取数据失败!QAQ', err_code: 9 })
...
// 数据重复校验
const validUnique = () => async (req, res, next) => {
  try {
    const doc = await req.Model.findOne(req.body)
